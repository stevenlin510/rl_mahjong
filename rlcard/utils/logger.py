...
class MultiLogger(object):
    ...
    def __init__(self, log_dir):
        ...
        Args:
            log_dir (str): The path to the log files
        '''
        self.log_dir = log_dir
    ...
    def __enter__(self):
        ...
        self.txt_file = open(self.txt_path, 'w')
        self.csv_file = open(self.csv_path, 'w')
        fieldnames = ['episode', 'agent_id', 'reward']
        self.writer = csv.DictWriter(self.csv_file, fieldnames=fieldnames)
        self.writer.writeheader()
    ...
    def log_performance(self, episode, agent_id, reward):
        ''' Log a point in the curve for a specific agent
        Args:
            episode (int): the episode of the current point
            agent_id (int): identifier of the agent
            reward (float): the reward of the current point
        '''
        self.writer.writerow({'episode': episode, 'agent_id': agent_id, 'reward': reward})
        print('')
        self.log('----------------------------------------')
        self.log(f'  episode      |  {episode}')
        self.log(f'  agent_id     |  {agent_id}')
